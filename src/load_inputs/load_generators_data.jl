...
function load_generators_data!(
    ...
    if !isempty(inputs_gen["RETRO"]) # If there are any retrofit technologies in consideration, read relevant data
        ...
        inputs_gen["RETROFIT_EFFICIENCIES"] = [
            [efficiencies[i][y] for i = 1:max_retro_sources if efficiencies[i][y] != 0]
            for y = 1:G
        ...
        inputs_gen["RETROFIT_INV_CAP_COSTS"] =
            [[inv_cap[i][y] for i = 1:max_retro_sources if inv_cap[i][y] >= 0] for y = 1:G]  # The set of investment costs (capacity $/MWyr) of each retrofit by source
    end
