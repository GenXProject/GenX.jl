...
function load_co2_load_side_emission_rate_cap(setup::Dict, path::AbstractString, sep::AbstractString, inputs_co2::Dict)
    ...
    inputs_co2["NCO2Cap_GenRate"] = cap

    # read in zone-cap membership
    first_col = findall(s -> s == Symbol("CO_2_Cap_Zone_1"), names(inputs_co2["dfCO2Cap_GenRate"]))[1]
    last_col = findall(s -> s == Symbol("CO_2_Cap_Zone_$cap"), names(inputs_co2["dfCO2Cap_GenRate"]))[1]
    inputs_co2["dfCO2GenRateCapZones"] = convert(Matrix{Float64}, inputs_co2["dfCO2Cap_GenRate"][:, first_col:last_col])

    # read in CO2 emissions cap in metric tons/MWh, with or without Scaling, the unit of emission cap (load rate) are both ton/MWh.
    first_col = findall(s -> s == Symbol("CO_2_Max_GenRate_1"), names(inputs_co2["dfCO2Cap_GenRate"]))[1]
    last_col = findall(s -> s == Symbol("CO_2_Max_GenRate_$cap"), names(inputs_co2["dfCO2Cap_GenRate"]))[1]
    inputs_co2["dfMaxCO2GenRate"] = convert(Matrix{Float64}, inputs_co2["dfCO2Cap_GenRate"][:, first_col:last_col])
