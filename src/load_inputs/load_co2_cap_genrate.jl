...
function load_co2_generation_side_emission_rate_cap(setup::Dict, path::AbstractString, inputs_co2::Dict)
    inputs_co2["dfCO2Cap_GenRate_slack"] = DataFrame(CSV.File(joinpath(path,"CO2_genrate_cap_slack.csv"), header=true), copycols=true)
    if setup["ParameterScale"] == 1
		inputs_co2["dfCO2Cap_GenRate_slack"][!,:PriceCap] ./= ModelScalingFactor #from $/ton to million$/kton.
	end
    inputs_co2["NCO2GenRateCap"] = size(collect(skipmissing(inputs_co2["dfCO2Cap_GenRate_slack"][!,:CO2_GenRate_Constraint])),1)
