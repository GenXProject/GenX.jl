...
function mga(
    ...
    if setup["ModelingToGenerateAlternatives"] == 1
        ...
        @constraint(
            ...
            vSumvP[tt, z] == sum(
                EP[:vP][y, t] * inputs["omega"][t] for y in dfGen[
                    (dfGen[!, :Resource_Type].==TechTypes[tt]).&(dfGen[!, :Zone].==z),
                    :R_ID,
                ], t = 1:T
            )
        )
        ...
        for i = 1:setup["ModelingToGenerateAlternativeIterations"]
            ...
            pRand = rand(
                length(unique(dfGen[dfGen[!, :MGA].==1, :Resource_Type])),
                length(unique(dfGen[!, :Zone])),
            )
