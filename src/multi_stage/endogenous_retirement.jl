...
function endogenous_retirement_discharge!(
    ...
    @expression(
        ...
        sum(
            ...
            p = 1:get_retirement_stage(cur_stage, dfGen[!, :Lifetime][y], stage_lens)
        )
    )
    @expression(
        EP,
        ...
        if y in COMMIT
            sum(
                (dfGen[!, Symbol("Min_Retired_Cap_MW")][y] / dfGen[!, :Cap_Size][y]) for
                p = 1:cur_stage
            )
        else
    ...
    @constraint(
        ...
        eNewCapTrack[y] + eMinRetCapTrack[y] <= eRetCapTrack[y]
    )
...
function endogenous_retirement_charge!(
    ...
    @expression(
        ...
        sum((dfGen[!, Symbol("Min_Retired_Charge_Cap_MW")][y]) for p = 1:cur_stage)
    )
...
function endogenous_retirement_energy!(
    ...
    @expression(
        ...
        sum((dfGen[!, Symbol("Min_Retired_Energy_Cap_MW")][y]) for p = 1:cur_stage)
    )
