...
function parse_data(myinputs)
    ...
    # LOAD - Load_data.csv
    load_profiles = [myinputs["pD"][:, l] for l = 1:size(myinputs["pD"], 2)]
    load_col_names = ["Load_MW_z" * string(l) for l = 1:size(load_profiles)[1]]
    load_zones = [l for l = 1:size(load_profiles)[1]]
    col_to_zone_map = Dict("Load_MW_z" * string(l) => l for l = 1:size(load_profiles)[1])
...
function parse_multi_stage_data(inputs_dict)
    ...
    vector_lps = [stage_load_profiles[:, l] for l = 1:size(inputs_dict[1]["pD"], 2)]
    load_profiles = [reduce(vcat, vector_lps[l]) for l = 1:size(inputs_dict[1]["pD"], 2)]
    load_col_names = ["Load_MW_z" * string(l) for l = 1:size(load_profiles)[1]]
    load_zones = [l for l = 1:size(load_profiles)[1]]
    col_to_zone_map = Dict("Load_MW_z" * string(l) => l for l = 1:size(load_profiles)[1])
...
function get_extreme_period(
    ...
    if typeKey == "Integral"
        if profKey == "Load"
            (stat, group_idx) =
                get_integral_extreme(GDF, statKey, load_col_names, ConstCols)
        elseif profKey == "PV"
            (stat, group_idx) =
        ...
        elseif profKey == "Wind"
            ...
            println(
                "Error: Profile Key ",
                profKey,
                " is invalid. Choose `Load', `PV' or `Wind'.",
            )
        end
...
function get_load_multipliers(
    ...
    for m = 1:NClusters
        clustered_lp_DF = DataFrame(
            Dict(
                ...
                for i = 1:Ncols if (Symbol(NewColNames[i]) in LoadCols)
            ),
        )
    ...
    for loadcol in LoadCols
        for m = 1:NClusters
            weighted_cluster_zone_sums[loadcol] +=
                (W[m] / (TimestepsPerRepPeriod)) * cluster_zone_sums[m][loadcol]
        end
            ...
            for i = 1:Ncols
                if (NewColNames[i] in LoadCols)
                    ...
                    println(
                        ...
                        " => ",
                        load_mults[NewColNames[i]] * sum(
                            ClusterOutputData[!, m][TimestepsPerRepPeriod*(i-1)+1:TimestepsPerRepPeriod*i],
                        ),
                    )
                    new_zone_sums[NewColNames[i]] +=
                        (W[m] / (TimestepsPerRepPeriod)) *
                        load_mults[NewColNames[i]] *
                        sum(
                            ClusterOutputData[!, m][TimestepsPerRepPeriod*(i-1)+1:TimestepsPerRepPeriod*i],
                        )
                end
...
function cluster_inputs(inpath, settings_path, mysetup, stage_id = -99, v = false)
    ...
    if LoadWeight != 1   # If we want to value load more/less than capacity factors. Assume nonnegative. LW=1 means no scaling.
        for c in load_col_names
            AnnualTSeriesNormalized[!, Symbol(c)] .=
                AnnualTSeriesNormalized[!, Symbol(c)] .* LoadWeight
        end
    end
                        ...
                        if ExtPeriodSelections[profKey][geoKey][typeKey][statKey] == 1
                            ...
                            elseif geoKey == "Zone"
                                for z in sort(unique(ZONES))
                                    ...
                                    if profKey == "Load"
                                        z_cols_type = intersect(z_cols, load_col_names)
                                    elseif profKey == "PV"
    ...
    if (Iterate == 1)
        while (
            ...
            if IterateMethod == "cluster"
                ...
                push!(
                    cluster_results,
                    cluster(ClusterMethod, ClusteringInputDF, NClusters, nReps, v),
                )
    ...
    LoadCols = [Symbol("Load_MW_z" * string(i)) for i = 1:length(load_col_names)]
    ...
    if !LoadExtremePeriod
        load_mults = get_load_multipliers(
            ClusterOutputData,
            InputData,
            M,
            W,
    ...
    if MultiStage == 1
        ...
        if MultiStageConcatenate == 1
            ...
            for per = 1:NumStages                      # Iterate over multi-stages
                ...
                load_in = load_dataframe(
                    joinpath(inpath, "Inputs", "Inputs_p$per", "Load_data.csv"),
                )
                load_in[!, :Sub_Weights] = load_in[!, :Sub_Weights] * 1.0
                ...
                Time_Index_M =
                    Union{Int64,Missings.Missing}[missing for i = 1:size(load_in, 1)]
                Time_Index_M[1:size(LPOutputData, 1)] = 1:size(LPOutputData, 1)
                load_in[!, :Time_Index] .= Time_Index_M

                for c in LoadCols
                    new_col =
                        Union{Float64,Missings.Missing}[missing for i = 1:size(load_in, 1)]
                ...
                GVColMap = Dict(
                    ...
                    i = 1:length(inputs_dict[1]["RESOURCES"])
                )
                ...
                fuel_in = load_dataframe(
                    joinpath(inpath, "Inputs", "Inputs_p$per", "Fuels_data.csv"),
                )
            ...
            load_in = load_dataframe(
                joinpath(inpath, "Inputs", input_stage_directory, "Load_data.csv"),
            )
            load_in[!, :Sub_Weights] = load_in[!, :Sub_Weights] * 1.0
            ...
            select!(load_in, Not(LoadCols))
            select!(load_in, Not(:Time_Index))
            Time_Index_M = Union{Int64,Missings.Missing}[missing for i = 1:size(load_in, 1)]
            Time_Index_M[1:size(LPOutputData, 1)] = 1:size(LPOutputData, 1)
            load_in[!, :Time_Index] .= Time_Index_M
            ...
            CSV.write(
                joinpath(inpath, "Inputs", input_stage_directory, Load_Outfile),
                load_in,
            )
            ...
            # Reset column ordering, add time index, and solve duplicate column name trouble with CSV.write's header kwarg
            GVColMap = Dict(
                RESOURCE_ZONES[i] => RESOURCES[i] for i = 1:length(myinputs["RESOURCES"])
            )
            GVColMap["Time_Index"] = "Time_Index"
            GVOutputData = GVOutputData[!, Symbol.(RESOURCE_ZONES)]
            insertcols!(GVOutputData, 1, :Time_Index => 1:size(GVOutputData, 1))
            ...
            fuel_in = load_dataframe(
                joinpath(inpath, "Inputs", input_stage_directory, "Fuels_data.csv"),
            )
            select!(fuel_in, Not(:Time_Index))
            SepFirstRow = DataFrame(fuel_in[1, :])
            NewFuelOutput = vcat(SepFirstRow, FPOutputData)
        ...
        for c in LoadCols
            new_col = Union{Float64,Missings.Missing}[missing for i = 1:size(load_in, 1)]
            new_col[1:size(LPOutputData, 1)] = LPOutputData[!, c]
            load_in[!, c] .= new_col
        end
        load_in = load_in[1:size(LPOutputData, 1), :]
        ...
        # Reset column ordering, add time index, and solve duplicate column name trouble with CSV.write's header kwarg
        GVColMap =
            Dict(RESOURCE_ZONES[i] => RESOURCES[i] for i = 1:length(myinputs["RESOURCES"]))
        GVColMap["Time_Index"] = "Time_Index"
        GVOutputData = GVOutputData[!, Symbol.(RESOURCE_ZONES)]
        insertcols!(GVOutputData, 1, :Time_Index => 1:size(GVOutputData, 1))
        NewGVColNames = [GVColMap[string(c)] for c in names(GVOutputData)]
