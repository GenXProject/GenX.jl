...
function ccs_solvent_storage!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @constraint(EP, cEmissionsCaptured_absorber[y in CCS_SOLVENT_STORAGE, t = 1:T],
                EP[:vOutput_CCS_SS][y, absorber, t] <= EP[:eEmissionsByPlant_gasturbine][y, t] * gen[y].co2_capture_fraction_max + 
                EP[:eEmissionsByPlant_Start_gasturbine][y, t] * gen[y].co2_capture_fraction_startup_max)
    ...
    # total captured emissions = capture fraction * total emissions
    @constraint(EP, cCapturedEmissionsTotal[y in CCS_SOLVENT_STORAGE],
                sum(omega[t] * EP[:vOutput_CCS_SS][y, regenerator, t] for t in 1:T) == 
                gen[y].co2_capture_fraction * sum(omega[t] * EP[:eEmissionsByPlant_gasturbine][y,t] + omega[t] * EP[:eEmissionsByPlant_Start_gasturbine][y,t] for t in 1:T))
