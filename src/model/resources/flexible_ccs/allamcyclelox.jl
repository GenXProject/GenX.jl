...
In this module, the key components of Allam cycle w/ LOX are break down into multiple components with independent capacity decisions. 

## Important expressions

**Power balance within an Allam Cycle resource**

Consumption of electricity by Air Separation Unit (ASU) $y, asu$ in time $t$, denoted by $\Pi_{y,asu,t}$, and auxiliary load, denoted by $\Pi_{y,aux,t}$, is subtracted from power generation from the sCO2 turbines, denoted by $\Pi_{y,sco2turbine,t}$
...
function allamcyclelox!(EP::Model, inputs::Dict, setup::Dict)
    ...
    WITH_LOX = inputs["WITH_LOX"]                                   # Set of Allam Cycle generators that can use liquid oxygen storage
    ...
    # y represents the plant, i represents the specific subcomponents, and t represents the time
    # The main output from sCO2Turbine/ASU/LOX is the gross power output from sCO2 cycle (MWh), power consumption associated with ASU (MWh), and the amount of LOX (tonne) stored in the LOX tank
    ...
    # Power consumption assumed by ASU is a function of generated LOX
    # Note: it should be noticed that the poweruserate to generate GOX and LOX are different.
    ...
    # The amount of LOX feed into oxyfuel cycle should be proportional to the power generated by oxyfuel cycle
    ...
    # Note: Allam Cycle is not compatible with RETRO for now.
    ...
    # Allam cycle is excluded from the constraint on vFuel in fuel.jl
    @constraint(EP, [y in ALLAM_CYCLE_LOX, t in 1:T], EP[:vFuel][y,t] == eFuel_Allam[y,t])
    
    # add vom
    # variable costs are related to the main output, e.g., gross power output from sCO2 turbine
    # power consumption associated with the ASU, and CO2 sequestration costs 
    # variable costs will be multiplied by inputs["omega"] to be compatible with time domain reduction
