...
function dry_cooler!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(EP, eMassFlow_Sec_Loop[y in UTES, t = 1:T],
        Q[t, gen[y].zone]/(gen[y].thermal_capacity_second_loop * (gen[y].temp_data_center_out-eTemp_HX_12[y,t]))
    )

    # thermal power of the dry cooler
    @expression(EP, eThermalPower_DC[y in UTES, t =1:T],
        gen[y].thermal_capacity_second_loop * EP[:eMassFlow_Sec_Loop][y, t] * (gen[y].temp_data_center_out-vTemp_DC[y,t]))

    # efficiency of the dry cooler
    @expression(EP, eCOP_DC[y in UTES, t = 1:T],
        1000*1.2*gen[y].fan_coefficient_dry_cooler * 1.013 * (gen[y].temp_data_center_out - gen[y].approach_temp_dry_cooler - pAmbientTemp[gen[y].zone, t])/(gen[y].fractional_pressure_dry_cooler_fan * gen[y].ambient_pressure_pa)
    )
