@doc raw"""
	utes_inter_period_linkage!(EP::Model, inputs::Dict, setup::Dict)
...
"""
function utes_inter_period_linkage!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @variable(EP, vSOC_UTESw[y in STOR_UTES_LONG_DURATION, n in MODELED_PERIODS_INDEX]>=0)

    # Build up in storage inventory over each representative period w
    # Build up inventory can be positive or negative
    @variable(EP, vdSOC_UTES[y in STOR_UTES_LONG_DURATION, w = 1:REP_PERIOD])
    ...
    if setup["LDSAdditionalConstraints"] == 1 && !isempty(NON_REP_PERIODS_INDEX)
	    ...
	    @variable(EP, vdSOC_maxPos_UTES[y in STOR_UTES_LONG_DURATION, w=1:REP_PERIOD] >= 0)

        # Maximum negative storage inventory change within subperiod
        @variable(EP, vdSOC_maxNeg_UTES[y in STOR_UTES_LONG_DURATION, w=1:REP_PERIOD] <= 0)
    end
    ...
    @constraint(EP,
        cUTESLongDurationStorageStart[w = 1:REP_PERIOD,
    ...
    # add self discharge??????
    @constraint(EP,
        cUTESLongDurationStorage[y in STOR_UTES_LONG_DURATION,
            r in MODELED_PERIODS_INDEX],
        vSOC_UTESw[y,
            mod1(r + 1, NPeriods)]==vSOC_UTESw[y, r] +
    ...
    @constraint(EP,
        cUTESLongDurationStorageSub[y in STOR_UTES_LONG_DURATION,
            r in REP_PERIODS_INDEX],
        vSOC_UTESw[y,r]==EP[:vSOC_RTES][y, hours_per_subperiod * dfPeriodMap[r, :Rep_Period_Index]] -
                vdSOC_UTES[y, dfPeriodMap[r, :Rep_Period_Index]])

    if setup["LDSAdditionalConstraints"] == 1 && !isempty(NON_REP_PERIODS_INDEX)
        # Extract maximum storage level variation (positive) within subperiod
        @constraint(EP, cMaxSoCVarPos_UTES[y in STOR_UTES_LONG_DURATION, w=1:REP_PERIOD, t=2:hours_per_subperiod],
                    vdSOC_maxPos_UTES[y,w] >= EP[:vSOC_RTES][y,hours_per_subperiod*(w-1)+t] - EP[:vSOC_RTES][y,hours_per_subperiod*(w-1)+1])

        # Extract maximum storage level variation (negative) within subperiod
        @constraint(EP, cMaxSoCVarNeg_UTES[y in STOR_UTES_LONG_DURATION, w=1:REP_PERIOD, t=2:hours_per_subperiod],
                        vdSOC_maxNeg_UTES[y,w] <= EP[:vSOC_RTES][y,hours_per_subperiod*(w-1)+t] - EP[:vSOC_RTES][y,hours_per_subperiod*(w-1)+1])
