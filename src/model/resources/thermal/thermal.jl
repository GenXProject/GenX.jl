...
function thermal!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(
        ...
        sum(
            EP[:vP][y, t] for
            y in intersect(inputs["THERM_ALL"], dfGen[dfGen[!, :Zone].==z, :R_ID])
        )
    )
    ...
    if setup["CapacityReserveMargin"] > 0
        @expression(
            EP,
            eCapResMarBalanceThermal[res = 1:inputs["NCapacityReserveMargin"], t = 1:T],
            sum(dfGen[y, Symbol("CapRes_$res")] * EP[:eTotalCap][y] for y in THERM_ALL)
        )
