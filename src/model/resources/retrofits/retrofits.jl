...
function retrofit(EP::Model, inputs::Dict)
    ...
    @expression(EP, eRetroRetireCap[y in RET_CAP], if y in COMMIT
    ...
    @expression(
        EP,
        eRetroRetireCapMap[y in RET_CAP],
        if y in COMMIT
            sum(
                EP[:vRETROFIT][y, r] * CAP_SIZE[y] for r in
                intersect(findall(x -> in(RESOURCES[y], RETRO_SOURCES[x]), 1:G), NEW_CAP);
                init = 0,
            )
        else
            sum(
                EP[:vRETROFIT][y, r] for r in
                intersect(findall(x -> in(RESOURCES[y], RETRO_SOURCES[x]), 1:G), NEW_CAP);
                init = 0,
            )
        end
    ...
    @expression(
        EP,
        eRetroInstallCapMap[r in intersect(RETRO, NEW_CAP)],
        if r in COMMIT
            sum(
                RETRO_SOURCE_IDS[r][i] in RET_CAP ?
                EP[:vRETROFIT][RETRO_SOURCE_IDS[r][i], r] *
                CAP_SIZE[RETRO_SOURCE_IDS[r][i]] *
                RETRO_EFFICIENCY[r][i] : 0 for i = 1:NUM_RETRO_SOURCES[r];
                init = 0,
            )
        else
            sum(
                RETRO_SOURCE_IDS[r][i] in RET_CAP ?
                EP[:vRETROFIT][RETRO_SOURCE_IDS[r][i], r] * RETRO_EFFICIENCY[r][i] : 0 for
                i = 1:NUM_RETRO_SOURCES[r];
                init = 0,
            )
        end
    )
    ...
    @expression(
        EP,
        eRetroInstallCap[r in intersect(RETRO, NEW_CAP)],
        if r in COMMIT
    ...
    @constraint(
        EP,
        cRetroDest[r in intersect(RETRO, NEW_CAP)],
        eRetroInstallCapMap[r] == eRetroInstallCap[r]
    )
