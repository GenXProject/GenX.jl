...
function thermal_storage!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if !isempty(FUS)
        fusion_systemwide_max_cap_constraint!(EP, inputs, setup)
...
function thermal_storage_core_commit_variables!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @variables(EP, begin
        vCCOMMIT[y in set, t in T] >= 0 #core commitment status
        vCSTART[y in set, t in T] >= 0 #core startup
        vCSHUT[y in set, t in T] >= 0 #core shutdown
    end)
    ...
    @constraints(EP, begin
        [y in set, t in T], vCCOMMIT[y,t] <= EP[:vCCAP][y] / by_rid(y,:Cap_Size)
        [y in set, t in T], vCSTART[y,t] <= EP[:vCCAP][y] / by_rid(y,:Cap_Size)
        [y in set, t in T], vCSHUT[y,t] <= EP[:vCCAP][y] / by_rid(y,:Cap_Size)
    end)
    ...
    @constraints(EP, begin
        ...
        [y in set, t in T], EP[:vCP][y,t] >= by_rid(y, :Min_Power) * by_rid(y, :Cap_Size) * vCCOMMIT[y,t]
        [y in set, t in T], EP[:vCP][y,t] <= by_rid(y, :Cap_Size) * vCCOMMIT[y,t]
    end)

    # Commitment state constraint linking startup and shutdown decisions (Constraint #4)
    @constraint(EP, [y in set, t in T],
        vCCOMMIT[y,t] == vCCOMMIT[y, hoursbefore(p,t,1)] + vCSTART[y,t] - vCSHUT[y,t])
end
...
function conventional_thermal_core_constraints!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if !isempty(COMMIT)
        ...
        ### Add startup costs ###
        @expression(EP, eCStartTS[y in COMMIT, t in T], (inputs["omega"][t] * inputs["TS_C_Start"][y][t] * vCSTART[y, t]))
        @expression(EP, eTotalCStartTST[t=T], sum(eCStartTS[y,t] for y in COMMIT))
        @expression(EP, eTotalCStartTS, sum(eTotalCStartTST[t] for t=T))
        EP[:eObj] += eTotalCStartTS


        #ramp up
        @constraint(EP,[t in T, y in COMMIT],
                    vCP[y,t]-vCP[y,hoursbefore(p, t, 1)] <= ramp_up_frac(y)*cap_size(y)*(vCCOMMIT[y,t]-vCSTART[y,t])
                    + min(1, max(min_power(y), ramp_up_frac(y)))*cap_size(y)*vCSTART[y,t]
                    - min_power(y) * cap_size(y) * vCSHUT[y,t])

        #ramp down
        @constraint(EP,[t in T, y in COMMIT],
                    vCP[y,hoursbefore(p, t, 1)]-vCP[y,t] <= ramp_dn_frac(y)*cap_size(y)*(vCCOMMIT[y,t]-vCSTART[y,t])
                    - min_power(y)*cap_size(y)*vCSTART[y,t]
                    + min(1,max(min_power(y), ramp_dn_frac(y)))*cap_size(y)*vCSHUT[y,t])
        ...
        @constraint(EP, [t in T, y in COMMIT],
            vCCAP[y]/cap_size(y)-vCCOMMIT[y,t] >= sum(vCSHUT[y, hoursbefore(p, t, 0:(down_time(y) - 1))])
        )
    end
end
...
function fusion_constraints!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @constraints(EP, begin
        ...
        [t in T, y in FUS], vCP[y,t] <= by_rid(y, :Cap_Size) * (vCCOMMIT[y,t] -
                                       by_rid(y, :Dwell_Time) * vCSTART[y,t])
    end)
    ...
    @constraint(EP, [y in FINITE_STARTS],
        sum(vCSTART[y,t]*inputs["omega"][t] for t in T) <=
    ...
    @constraint(EP, [t in T, y in MAX_UPTIME],
            vCCOMMIT[y,t] <= sum(vCSTART[y, hoursbefore(p, t, 0:(max_uptime(y)-1))]))
...
function maintenance_constraints!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @variables(EP, begin
        vMDOWN[y in MAINTENANCE, t in T] >= 0  # core maintenance status
        vMSHUT[y in MAINTENANCE, t=maintenance_begin_hours] >= 0  # core maintenance shutdown
    end)
    ...
    @constraints(EP, begin
        [y in MAINTENANCE, t in T], vMDOWN[y,t] <= vCCAP[y] / by_rid(y,:Cap_Size)
        [y in MAINTENANCE, t=maintenance_begin_hours], vMSHUT[y,t] <= vCCAP[y] / by_rid(y,:Cap_Size)
    end)

    # Require plant to shut down during maintenance
    @constraint(EP, [y in MAINTENANCE, t in T],
        vCCAP[y] / by_rid(y,:Cap_Size) - vCCOMMIT[y,t] >= vMDOWN[y,t])

    controlling_hours(t,y) = controlling_maintenance_start_hours(hours_per_subperiod, t, maint_dur[y], maintenance_begin_hours)
    @constraint(EP, [y in MAINTENANCE, t in T],
                vMDOWN[y,t] == sum(vMSHUT[y, controlling_hours(t,y)]))

    @constraint(EP, [y in MAINTENANCE],
        sum(vMSHUT[y,t]*weights[t] for t in maintenance_begin_hours) >= vCCAP[y] / by_rid(y,:Maintenance_Cadence_Years) / by_rid(y,:Cap_Size))

    for y in intersect(FUS, MAINTENANCE), t in T
            add_to_expression!(EP[:ePassiveRecircFus][y,t],
                               -by_rid(y,:Cap_Size) * vMDOWN[y,t] * dfGen[y,:Eff_Down] * by_rid(y,:Recirc_Pass))
    end
end
