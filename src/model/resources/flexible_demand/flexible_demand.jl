...
@expression(EP, eCVarFlex_in[y in FLEX, t= 1:T],dfGen[y, :Var_OM_Cost_per_MWh_In] * vCHARGE_FLEX[y, t])

# Sum to Plant level
@expression(EP, ePlantCVarFlexIn[y in FLEX],  inputs["omega"][t] * sum(EP[:eCVarFlex_in][y, t] for t in 1:T))
# Sum to zonal level
