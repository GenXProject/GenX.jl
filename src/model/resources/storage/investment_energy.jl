...
function investment_energy!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(
        ...
        if y in NEW_CAP_ENERGY # Resources eligible for new capacity
            dfGen[y, :Inv_Cost_per_MWhyr] * vCAPENERGY[y] +
    ...
    if MultiStage == 1
        @constraint(
            ...
            EP[:vEXISTINGCAPENERGY][y] == dfGen[y, :Existing_Cap_MWh]
        )
    end
    ...
    @constraint(
        EP,
        cMaxCapEnergy[y in intersect(dfGen[dfGen.Max_Cap_MWh.>0, :R_ID], STOR_ALL)],
        eTotalCapEnergy[y] <= dfGen[y, :Max_Cap_MWh]
    )
    ...
    @constraint(
        EP,
        cMinCapEnergy[y in intersect(dfGen[dfGen.Min_Cap_MWh.>0, :R_ID], STOR_ALL)],
        eTotalCapEnergy[y] >= dfGen[y, :Min_Cap_MWh]
    )
    ...
    @constraint(
        ...
        EP[:eTotalCapEnergy][y] >= dfGen[y, :Min_Duration] * EP[:eTotalCap][y]
    )
    @constraint(
        EP,
        cMaxCapEnergyDuration[y in STOR_ALL],
        EP[:eTotalCapEnergy][y] <= dfGen[y, :Max_Duration] * EP[:eTotalCap][y]
    )
