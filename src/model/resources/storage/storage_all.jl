...
function storage_all!(EP::Model, inputs::Dict, setup::Dict)
	...
	@expression(EP, eCVar_in[y in STOR_ALL,t=1:T], inputs["omega"][t]*var_om_cost_per_mwh_in(res[y])*vCHARGE[y,t])
	...
	@expression(EP, ePowerBalanceStor[t=1:T, z=1:Z],
		sum(EP[:vP][y,t]-EP[:vCHARGE][y,t] for y in intersect(resources_in_zone_by_rid(res,z),STOR_ALL))
	)
	...
	@constraint(EP, cSoCBalStart[t in START_SUBPERIODS, y in CONSTRAINTSET], EP[:vS][y,t] ==
		EP[:vS][y,t+hours_per_subperiod-1] - (1/efficiency_down(res[y]) * EP[:vP][y,t])
		+ (efficiency_up(res[y])*EP[:vCHARGE][y,t]) - (self_discharge(res[y]) * EP[:vS][y,t+hours_per_subperiod-1]))
	...
	@constraints(EP, begin
		...
		cSoCBalInterior[t in INTERIOR_SUBPERIODS, y in STOR_ALL], EP[:vS][y,t] ==
			EP[:vS][y,t-1]-(1/efficiency_down(res[y])*EP[:vP][y,t])+(efficiency_up(res[y])*EP[:vCHARGE][y,t])-(self_discharge(res[y])*EP[:vS][y,t-1])
	end)
	...
	if Reserves == 1
		...
		if CapacityReserveMargin > 0
			...
			@constraints(EP, begin
				...
				[y in STOR_ALL, t=1:T], EP[:vP][y,t] + EP[:vCAPRES_discharge][y,t] <= EP[:vS][y, hoursbefore(hours_per_subperiod,t,1)]*efficiency_down(res[y])
			end)
		else
			@constraints(EP, begin
				[y in STOR_ALL, t=1:T], EP[:vP][y,t] <= EP[:eTotalCap][y]
				[y in STOR_ALL, t=1:T], EP[:vP][y,t] <= EP[:vS][y, hoursbefore(hours_per_subperiod,t,1)]*efficiency_down(res[y])
			end)
		end
	end

	# From CO2 Policy module
	expr = @expression(EP, [z=1:Z], sum(EP[:eELOSS][y] for y in intersect(STOR_ALL, resources_in_zone_by_rid(res,z))))
	...
	if CapacityReserveMargin > 0
		...
		@constraint(EP, cVSoCBalStart[t in START_SUBPERIODS, y in CONSTRAINTSET], EP[:vCAPRES_socinreserve][y,t] ==
			EP[:vCAPRES_socinreserve][y,t+hours_per_subperiod-1] + (1/efficiency_down(res[y]) * EP[:vCAPRES_discharge][y,t])
			- (efficiency_up(res[y])*EP[:vCAPRES_charge][y,t]) - (self_discharge(res[y]) * EP[:vCAPRES_socinreserve][y,t+hours_per_subperiod-1]))
		...
		@constraint(EP, cVSoCBalInterior[t in INTERIOR_SUBPERIODS, y in STOR_ALL], EP[:vCAPRES_socinreserve][y,t] ==
			EP[:vCAPRES_socinreserve][y,t-1]+(1/efficiency_down(res[y])*EP[:vCAPRES_discharge][y,t])-(efficiency_up(res[y])*EP[:vCAPRES_charge][y,t])-(self_discharge(res[y])*EP[:vCAPRES_socinreserve][y,t-1]))
...
function storage_all_reserves!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @constraint(EP, [y in STOR_REG, t in 1:T], vREG[y, t] <= reg_max(res[y]) * eTotalCap[y])
    @constraint(EP, [y in STOR_RSV, t in 1:T], vRSV[y, t] <= rsv_max(res[y]) * eTotalCap[y])
    ...
    @constraint(EP, [y in STOR_REG, t in 1:T], efficiency_up(res[y])*(vCHARGE[y, t]+vREG_charge[y, t]) <= eTotalCapEnergy[y]-vS[y, hoursbefore(p,t,1)])
    # Note: maximum charge rate is also constrained by maximum charge power capacity, but as this differs by storage type,
    # this constraint is set in functions below for each storage type
    ...
    @constraint(EP, [y in STOR_ALL, t in 1:T], expr[y, t] <= vS[y, hoursbefore(p,t,1)] * efficiency_down(res[y]))
end
