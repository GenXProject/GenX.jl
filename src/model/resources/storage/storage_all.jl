...
function storage_all!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(
        ...
        inputs["omega"][t] * dfGen[y, :Var_OM_Cost_per_MWh_In] * vCHARGE[y, t]
    )
    ...
    @constraint(
        ...
        (dfGen[y, :Self_Disch] * EP[:vS][y, t+hours_per_subperiod-1])
    )
    ...
    @constraints(
        ...
        begin
            ...
            EP[:vS][y, t] ==
            EP[:vS][y, t-1] - (1 / dfGen[y, :Eff_Down] * EP[:vP][y, t]) +
            (dfGen[y, :Eff_Up] * EP[:vCHARGE][y, t]) -
            (dfGen[y, :Self_Disch] * EP[:vS][y, t-1])
        end
    )
    ...
    if Reserves == 1
        ...
        @constraints(
            ...
            begin
                ...
                [y in STOR_ALL, t = 1:T],
                EP[:vP][y, t] <=
                EP[:vS][y, hoursbefore(hours_per_subperiod, t, 1)] * dfGen[y, :Eff_Down]
            end
        )
    end
    ...
    if !isempty(STOR_REG_RSV)
        ...
        @constraints(
            ...
            begin
                ...
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vREG][y, t] <= dfGen[y, :Reg_Max] * EP[:eTotalCap][y]
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vRSV][y, t] <= dfGen[y, :Rsv_Max] * EP[:eTotalCap][y]
                ...
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vREG][y, t] == EP[:vREG_charge][y, t] + EP[:vREG_discharge][y, t]
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vRSV][y, t] == EP[:vRSV_charge][y, t] + EP[:vRSV_discharge][y, t]
                ...
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vCHARGE][y, t] - EP[:vREG_charge][y, t] - EP[:vRSV_charge][y, t] >= 0
                ...
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vP][y, t] - EP[:vREG_discharge][y, t] >= 0

                # Maximum charging rate plus contribution to regulation down must be less than available storage capacity
                [y in STOR_REG_RSV, t = 1:T],
                dfGen[y, :Eff_Up] * (EP[:vCHARGE][y, t] + EP[:vREG_charge][y, t]) <=
                EP[:eTotalCapEnergy][y] - EP[:vS][y, hoursbefore(p, t, 1)]
                # Note: maximum charge rate is also constrained by maximum charge power capacity, but as this differs by storage type,
                # this constraint is set in functions below for each storage type
                ...
                # wrapping from end of sample period to start of sample period for energy capacity constraint
                [y in STOR_REG_RSV, t = 1:T],
                EP[:vP][y, t] + EP[:vREG_discharge][y, t] + EP[:vRSV_discharge][y, t] <=
                EP[:eTotalCap][y]
                [y in STOR_REG_RSV, t = 1:T],
                (EP[:vP][y, t] + EP[:vREG_discharge][y, t] + EP[:vRSV_discharge][y, t]) /
                dfGen[y, :Eff_Down] <= EP[:vS][y, hoursbefore(p, t, 1)]
            end
        )
    end
    if !isempty(STOR_REG_ONLY)
        ...
        @constraints(
            ...
            begin
                ...
                # Note: when charging, reducing charge rate is contributing to upwards reserve & regulation as it drops net demand
                [y in STOR_REG_ONLY, t = 1:T],
                EP[:vCHARGE][y, t] - EP[:vREG_charge][y, t] >= 0
                ...
                # wrapping from end of sample period to start of sample period for energy capacity constraint
                [y in STOR_REG_ONLY, t = 1:T],
                EP[:vP][y, t] + EP[:vREG_discharge][y, t] <= EP[:eTotalCap][y]
                [y in STOR_REG_ONLY, t = 1:T],
                (EP[:vP][y, t] + EP[:vREG_discharge][y, t]) / dfGen[y, :Eff_Down] <=
                EP[:vS][y, hoursbefore(p, t, 1)]
            end
        )
    end
    if !isempty(STOR_RSV_ONLY)
        ...
        @constraints(
            ...
            begin
                ...
                # Note: when charging, reducing charge rate is contributing to upwards reserve & regulation as it drops net demand
                [y in STOR_RSV_ONLY, t = 1:T],
                EP[:vCHARGE][y, t] - EP[:vRSV_charge][y, t] >= 0
                ...
                # wrapping from end of sample period to start of sample period for energy capacity constraint
                [y in STOR_RSV_ONLY, t = 1:T],
                EP[:vP][y, t] + EP[:vRSV_discharge][y, t] <= EP[:eTotalCap][y]
                [y in STOR_RSV_ONLY, t = 1:T],
                (EP[:vP][y, t] + EP[:vRSV_discharge][y, t]) / dfGen[y, :Eff_Down] <=
                EP[:vS][y, hoursbefore(p, t, 1)]
            end
        )
    end
    if !isempty(STOR_NO_RES)
        ...
        @constraints(
            ...
            begin
                ...
                [y in STOR_NO_RES, t = 1:T],
                EP[:vP][y, t] / dfGen[y, :Eff_Down] <= EP[:vS][y, hoursbefore(p, t, 1)]
            end
        )
    end
end
