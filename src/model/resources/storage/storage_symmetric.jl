...
function storage_symmetric_reserves!(EP::Model, inputs::Dict)
    ...
    if !isempty(STOR_SYM_REG_RSV)
        ...
        @constraints(
            ...
            begin
                ...
                [y in STOR_SYM_REG_RSV, t in 1:T],
                EP[:vCHARGE][y, t] + EP[:vREG_charge][y, t] <= EP[:eTotalCap][y]

                # Max simultaneous charge and discharge rates cannot be greater than symmetric charge/discharge capacity
                [y in STOR_SYM_REG_RSV, t in 1:T],
                EP[:vP][y, t] +
                EP[:vREG_discharge][y, t] +
                EP[:vRSV_discharge][y, t] +
                EP[:vCHARGE][y, t] +
                EP[:vREG_charge][y, t] <= EP[:eTotalCap][y]
            end
        )
    end
    ...
    if !isempty(STOR_SYM_REG_ONLY)
        ...
        @constraints(
            ...
            begin
                # Maximum charging rate plus contribution to regulation down must be less than symmetric power rating
                [y in STOR_SYM_REG_ONLY, t in 1:T],
                EP[:vCHARGE][y, t] + EP[:vREG_charge][y, t] <= EP[:eTotalCap][y]
    ...
    if !isempty(STOR_SYM_RSV_ONLY)
        ...
        @constraints(
            ...
            begin
                ...
                # Max simultaneous charge and discharge rates cannot be greater than symmetric charge/discharge capacity
                [y in STOR_SYM_RSV_ONLY, t in 1:T],
                EP[:vP][y, t] + EP[:vRSV_discharge][y, t] + EP[:vCHARGE][y, t] <=
                EP[:eTotalCap][y]
            end
        )
    end
