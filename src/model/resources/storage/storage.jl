...
If a capacity reserve margin is modeled, variables for virtual charge, $c^{charge}_{o,z,t}$, and virtual discharge, $c^{discharge}_{o,z,t}$, are created to represent contributions that a storage device makes to the capacity reserve margin without actually generating power. These represent power that the storage device could have discharged or consumed if called upon to do so, based on its available state of charge. Importantly, a dedicated set of variables (those of the form $c^{...}_{o,z,t}$) and constraints are created to ensure that any virtual contributions to the capacity reserve margin could be made as actual charge/discharge if necessary without affecting system operations in any other timesteps. If a capacity reserve margin is not modeled, all related variables are fixed at 0. The overall contribution of storage devices to the system's capacity reserve margin in timestep $t$ is equal to $\sum_{y \in \mathcal{O}} \epsilon_{y,z,p}^{CRM} \times \left(\Theta_{y,z,t} + c^{discharge}_{o,z,t} - c^{charge}_{o,z,t} - \Pi_{y,z,t} \right)$, and includes both actual and virtual charge and discharge.
```math
\begin{aligned}
	&  \Pi_{o,z,t} + c^{charge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}^{sym}, z \in \mathcal{Z}, t \in \mathcal{T}\\
	&  \Pi_{o,z,t} + c^{charge}_{o,z,t} + \Theta_{o,z,t} + c^{discharge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}^{sym}, z \in \mathcal{Z}, t \in \mathcal{T}
\end{aligned}
```
**Storage with symmetric charge and discharge capacity**
For storage technologies with symmetric charge and discharge capacity (all $o \in \mathcal{O}^{sym}$), charge rate, $\Pi_{o,z,t}$, and virtual charge rate, $c^{charge}_{o,z,t}$, are jointly constrained by the total installed power capacity, $\Omega_{o,z}$. Since storage resources generally represent a `cluster' of multiple similar storage devices of the same type/cost in the same zone, GenX permits storage resources to simultaneously charge and discharge (as some units could be charging while others discharge), with the simultaenous sum of charge, $\Pi_{o,z,t}$, discharge, $\Theta_{o,z,t}$, virtual charge, $c^{charge}_{o,z,t}$, and virtual discharge, $c^{discharge}_{o,z,t}$, also limited by the total installed power capacity, $\Delta^{total}_{o,z}$. These two constraints are as follows:
```math
\begin{aligned}
	&  \Pi_{o,z,t} + c^{charge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}^{sym}, z \in \mathcal{Z}, t \in \mathcal{T}\\
	&  \Pi_{o,z,t} + c^{charge}_{o,z,t} + \Theta_{o,z,t} + c^{discharge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}^{sym}, z \in \mathcal{Z}, t \in \mathcal{T}
...
```math
\begin{aligned}
&  \Pi_{o,z,t} + c^{charge}_{o,z,t} + f^{charge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}^{sym}, z \in \mathcal{Z}, t \in \mathcal{T}\\
&  \Pi_{o,z,t} + c^{charge}_{o,z,t} + f^{charge}_{o,z,t} + \Theta_{o,z,t} + c^{discharge}_{o,z,t} + f^{discharge}_{o,z,t} + r^{discharge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}^{sym}, z \in \mathcal{Z}, t \in \mathcal{T} \\
\end{aligned}
```
...
```math
\begin{aligned}
	&  \Pi_{o,z,t} + c^{charge}_{o,z,t} \leq \Delta^{total, charge}_{o,z} & \quad \forall o \in \mathcal{O}^{asym}, z \in \mathcal{Z}, t \in \mathcal{T}
\end{aligned}
```
...
```math
\begin{aligned}
	&  \Pi_{o,z,t} + c^{charge}_{o,z,t} + f^{charge}_{o,z,t} \leq \Delta^{total, charge}_{o,z} & \quad \forall o \in \mathcal{O}^{asym}, z \in \mathcal{Z}, t \in \mathcal{T}
\end{aligned}
```
...
\end{aligned}
```
If a capacity reserve margin is modeled, then the following constraints track the relationship between the virtual charge, $c^{charge}_{o,z,t}$, and virtual discharge, $c^{discharge}_{o,z,t}$, variables and a third variable, $c^{socinreserve}_{o,z,t}$, representing the amount of state of charge that must be held in reserve to enable these virtual capacity reserve margin contributions, ensuring that the storage device could deliver its pledged capacity if called upon to do so without affecting its operations in other timesteps. $c^{socinreserve}_{o,z,t}$ is tracked similarly to the devices overall state of charge based on its value in the previous timestep and the virtual charge and discharge in the current timestep. Unlike the regular state of charge, virtual discharge $c^{discharge}_{o,z,t}$ increases $c^{socinreserve}_{o,z,t}$ (as more charge must be held in reserve to support more virtual discharge), and $c^{charge}_{o,z,t}$ reduces $c^{socinreserve}_{o,z,t}$.
```math
\begin{aligned}
	&  c^{socinreserve}_{o,z,t} =c^{socinreserve}_{o,z,t-1} + \frac{1}{\eta_{o,z}^{discharge}}c^{discharge}_{o,z,t} - \eta_{o,z}^{charge}c^{charge}_{o,z,t} - \eta_{o,z}^{loss}c^{socinreserve}_{o,z,t-1}  \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}^{interior}\\
	&  c^{socinreserve}_{o,z,t} =c^{socinreserve}_{o,z,t+\tau^{period}-1} + \frac{1}{\eta_{o,z}^{discharge}}c^{discharge}_{o,z,t} - \eta_{o,z}^{charge}c^{charge}_{o,z,t} - \eta_{o,z}^{loss}c^{socinreserve}_{o,z,t+\tau^{period}-1}  \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}^{start}
\end{aligned}
```
The energy held in reserve, $c^{discharge}_{o,z,t}$, also acts as a lower bound on the overall state of charge $\Gamma_{o,z,t}$. This ensures that the storage device cannot use state of charge that would not have been available had it been called on to actually contribute its pledged virtual discharge at some earlier timestep. This relationship is described by the following equation:
```math
\begin{aligned}
	&  \Gamma_{o,z,t} \geq c^{socinreserve}_{o,z,t} 
\end{aligned}
```
...
Finally, the maximum combined discharge and virtual discharge rate for storage resources, $\Pi_{o,z,t} + c^{discharge}_{o,z,t}$, is constrained to be less than the discharge power capacity, $\Omega_{o,z,t}$ or the state of charge at the end of the last period, $\Gamma_{o,z,t-1}$, whichever is less.
```math
\begin{aligned}
	&  \Gamma_{o,z,t} \leq \Delta^{total, energy}_{o,z} & \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}\\
	&  \Theta_{o,z,t} + c^{discharge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}\\
	&  \Theta_{o,z,t} + c^{discharge}_{o,z,t} \leq \Gamma_{o,z,t-1} & \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}
\end{aligned}
```
...
```math
\begin{aligned}
	&  \eta_{o,z}^{charge} \times (\Pi_{o,z,t} + c^{charge}_{o,z,t} + f^{charge}_{o,z,t}) \leq \Delta^{energy, total}_{o,z} - \Gamma_{o,z,t-1} & \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}
\end{aligned}
```
...
```math
\begin{aligned}
	&  \Theta_{o,z,t} + c^{discharge}_{o,z,t} + f^{discharge}_{o,z,t} + r^{discharge}_{o,z,t} \leq \Delta^{total}_{o,z} & \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}\\
	&  \Theta_{o,z,t} + c^{discharge}_{o,z,t} + f^{discharge}_{o,z,t} + r^{discharge}_{o,z,t} \leq \Gamma_{o,z,t-1} & \quad \forall o \in \mathcal{O}, z \in \mathcal{Z}, t \in \mathcal{T}
\end{aligned}
```
