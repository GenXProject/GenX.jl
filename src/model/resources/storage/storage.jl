...
If there is more than one representative period and ```LDS``` has been enabled for resources in ```Generators```, this function calls ```long_duration_storage()``` in ```long_duration_storage.jl``` to enable this feature.
