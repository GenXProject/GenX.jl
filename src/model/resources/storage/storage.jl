...
function storage!(EP::Model, inputs::Dict, setup::Dict)
	...
	if EnergyShareRequirement >= 1
		if IncludeLossesInESR == 1
			@expression(EP, eESRStor[ESR=1:inputs["nESR"]], sum(inputs["dfESR"][z,ESR]*sum(EP[:eELOSS][y] for y in intersect(resources_in_zone_by_rid(res,z),STOR_ALL)) for z=findall(x->x>0,inputs["dfESR"][:,ESR])))
