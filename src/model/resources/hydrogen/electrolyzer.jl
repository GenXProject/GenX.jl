...
function electrolyzer!(EP::Model, inputs::Dict, setup::Dict)
	...
	electrolyzer_min_kt(y) = electrolyzer_min_kt(resources[y])
	hydrogen_mwh_per_tonne(y) = hydrogen_mwh_per_tonne(resources[y])
	hydrogen_price_per_tonne(y) = hydrogen_price_per_tonne(resources[y])
	qualified_hydrogen_supply(y) = qualified_hydrogen_supply(resources[y])
	...
	sum(EP[:vUSE][y,t] for y in intersect(ELECTROLYZERS, resources_in_zone_by_rid(resources,z))))
	...
	@constraints(EP, begin
        ...
        [y in ELECTROLYZERS, t in 1:T], EP[:vUSE][y,t] - EP[:vUSE][y, hoursbefore(p,t,1)] <= ramp_up_percentage(y)*EP[:eTotalCap][y]

		## Maximum ramp down between consecutive hours
		[y in ELECTROLYZERS, t in 1:T], EP[:vUSE][y, hoursbefore(p,t,1)] - EP[:vUSE][y,t] <= ramp_down_percentage(y)*EP[:eTotalCap][y]
	end)
    ...
    @constraints(EP, begin
		...
		[y in ELECTROLYZERS, t in 1:T], EP[:vUSE][y,t] >= min_power(y)*EP[:eTotalCap][y]
	...
	@constraint(EP,
		...
		sum(inputs["omega"][t] * EP[:vUSE][y,t] / hydrogen_mwh_per_tonne(y) for t=1:T) >= electrolyzer_min_kt(y) * kt_to_t
	)
	...
	if setup["HydrogenHourlyMatching"] == 1
		HYDROGEN_ZONES = unique(zone_id.(resources.ELECTROLYZER))
		QUALIFIED_SUPPLY = resources_with_qualified_hydrogen_supply(resources)
		@constraint(EP, cHourlyMatching[z in HYDROGEN_ZONES, t in 1:T],
			sum(EP[:vP][y,t] for y=intersect(resources_in_zone_by_rid(resources,z), QUALIFIED_SUPPLY)) >= sum(EP[:vUSE][y,t] for y=intersect(resources_in_zone_by_rid(resources,z), ELECTROLYZERS)) + sum(EP[:vCHARGE][y,t] for y=intersect(resources_in_zone_by_rid(resources,z), QUALIFIED_SUPPLY, STORAGE))
		)
	end
	...
	@expression(EP, eHydrogenValue[y in ELECTROLYZERS, t in 1:T], (inputs["omega"][t] * EP[:vUSE][y,t] / hydrogen_mwh_per_tonne(y) * hydrogen_price_per_tonne(y) / scale_factor))
