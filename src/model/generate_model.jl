...
function operation_model!(EP::Model,setup::Dict, inputs::Dict)
    ...
    if !haskey(EP,:eTotalCap) 
        # This only gets run if the planning model has not been built on EP
        # This function is specifically for supporting Benders decomposition
    ...
    if !isempty(inputs["ELECTROLYZER"])
        error("Electrolyzers are not yet supported on this branch of GenX; please open an issue or use a different version")
    ...
    if Z > 1 && (setup["DC_OPF"] == 0 || setup["NetworkExpansion"] == 0)
...
function planning_model!(EP::Model,setup::Dict, inputs::Dict)
    ...
    if setup["HydrogenMinimumProduction"] > 0
        if setup["Benders"] == 1
            error("Hydrogen Minimum Production Not yet Supported with Benders")
        else
