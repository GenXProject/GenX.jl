...
function investment_discharge!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["MinCapReq"] == 1
        @expression(EP,
            ...
            sum(EP[:eTotalCap][y] for y in setdiff(ids_with_policy(gen, min_cap, tag = mincap), CCS_SOLVENT_STORAGE)))
    ...
    if setup["MaxCapReq"] == 1
        @expression(EP,
            ...
            sum(EP[:eTotalCap][y] for y in setdiff(ids_with_policy(gen, max_cap, tag = maxcap), CCS_SOLVENT_STORAGE)))
