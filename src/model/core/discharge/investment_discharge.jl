...
function investment_discharge(EP::Model, inputs::Dict, MinCapReq::Int)
	...
	@expression(EP, eTotalCap[y in 1:G],
		if y in intersect(NEW_CAP, RET_CAP) # Resources eligible for new capacity and retirements
			if y in COMMIT
				dfGen[y,:Existing_Cap_MW] + dfGen[y,:Cap_Size]*(EP[:vCAP][y] - EP[:vRETCAP][y])
			else
				dfGen[y,:Existing_Cap_MW] + EP[:vCAP][y] - EP[:vRETCAP][y]
			end
		elseif y in setdiff(NEW_CAP, RET_CAP) # Resources eligible for only new capacity
			if y in COMMIT
				dfGen[y,:Existing_Cap_MW] + dfGen[y,:Cap_Size]*EP[:vCAP][y]
			else
				dfGen[y,:Existing_Cap_MW] + EP[:vCAP][y]
			end
		elseif y in setdiff(RET_CAP, NEW_CAP) # Resources eligible for only capacity retirements
			if y in COMMIT
				dfGen[y,:Existing_Cap_MW] - dfGen[y,:Cap_Size]*EP[:vRETCAP][y]
			else
				dfGen[y,:Existing_Cap_MW] - EP[:vRETCAP][y]
			end
		else # Resources not eligible for new capacity or retirements
			dfGen[y,:Existing_Cap_MW] + EP[:vZERO]
		end
	)
	...
	@constraint(EP, cMaxRetNoCommit[y in setdiff(RET_CAP,COMMIT)], vRETCAP[y] <= dfGen[y,:Existing_Cap_MW])
	@constraint(EP, cMaxRetCommit[y in intersect(RET_CAP,COMMIT)], dfGen[y,:Cap_Size]*vRETCAP[y] <= dfGen[y,:Existing_Cap_MW])
