...
function investment_discharge!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if !isempty(RETRO)
        ...
        ALL_SOURCES =
            intersect(collect(Set(collect(Iterators.flatten(RETRO_SOURCE_IDS)))), RET_CAP)
        DESTS_BY_SOURCE = [
            y in ALL_SOURCES ?
            intersect(
                findall(x -> in(inputs["RESOURCES"][y], RETRO_SOURCES[x]), 1:G),
                findall(x -> x in NEW_CAP, 1:G),
            ) : [] for y = 1:G
        ]
    ...
    @expression(
        ...
        elseif y in intersect(NEW_CAP, RETRO) # Resources eligible for new capacity (Retrofit yr -> y)
            if y in COMMIT
                sum(
                    RETRO_SOURCE_IDS[y][i] in RET_CAP ?
                    RETRO_INV_CAP_COSTS[y][i] *
                    dfGen[y, :Cap_Size] *
                    vRETROFIT[RETRO_SOURCE_IDS[y][i], y] *
                    RETRO_EFFICIENCY[y][i] : 0 for i = 1:NUM_RETRO_SOURCES[y]
                ) + dfGen[y, :Fixed_OM_Cost_per_MWyr] * eTotalCap[y]
            else
                sum(
                    RETRO_SOURCE_IDS[y][i] in RET_CAP ?
    ...
    if MultiStage == 1
        ...
        @constraint(
            ...
            EP[:vEXISTINGCAP][y] == dfGen[y, :Existing_Cap_MW]
        )
    end
    ...
    @constraint(
        ...
        dfGen[y, :Cap_Size] * vRETCAP[y] <= eExistingCap[y]
    )
    ...
    @constraint(
        EP,
        cMaxCap[y in intersect(dfGen[dfGen.Max_Cap_MW.>0, :R_ID], 1:G)],
        eTotalCap[y] <= dfGen[y, :Max_Cap_MW]
    )
    ...
    @constraint(
        EP,
        cMinCap[y in intersect(dfGen[dfGen.Min_Cap_MW.>0, :R_ID], 1:G)],
        eTotalCap[y] >= dfGen[y, :Min_Cap_MW]
    )
    ...
    if setup["MinCapReq"] == 1
        @expression(
            ...
            sum(
                EP[:eTotalCap][y] for
                y in dfGen[dfGen[!, Symbol("MinCapTag_$mincap")].==1, :R_ID]
            )
        )
    ...
    if setup["MaxCapReq"] == 1
        @expression(
            ...
            sum(
                EP[:eTotalCap][y] for
                y in dfGen[dfGen[!, Symbol("MaxCapTag_$maxcap")].==1, :R_ID]
            )
        )
