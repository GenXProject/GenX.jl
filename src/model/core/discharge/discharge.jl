...
function discharge!(EP::Model, inputs::Dict, setup::Dict)
    ...
    @expression(
        ...
        (
            inputs["omega"][t] *
            (dfGen[y, :Var_OM_Cost_per_MWh] + inputs["C_Fuel_per_MWh"][y, t]) *
            vP[y, t]
        )
    )
    ...
    if setup["EnergyShareRequirement"] >= 1
        ...
        @expression(
            ...
            sum(
                inputs["omega"][t] * dfGen[y, Symbol("ESR_$ESR")] * EP[:vP][y, t] for
                y in dfGen[findall(x -> x > 0, dfGen[!, Symbol("ESR_$ESR")]), :R_ID],
                t = 1:T
            ) - sum(
