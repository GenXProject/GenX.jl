...
@doc raw"""
	write_esr_storagelosspayment(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, dfPower::DataFrame, dfESR::DataFrame)
...
"""
function write_esr_storagelosspayment(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    Z = inputs["Z"]
    nESR = inputs["nESR"]
    dfESRStoragelosspayment = DataFrame(Zone = 1:Z, AnnualSum = zeros(Z))
    tempesrpayment = zeros(Z, nESR)
    tempesrpayment = (inputs["dfESR"] .* (value.(EP[:eELOSSByZone]))) .* repeat(transpose(dual.(EP[:cESRShare])), Z, 1)
    if setup["ParameterScale"] == 1
        tempesrpayment = tempesrpayment * (ModelScalingFactor^2)
    end
    dfESRStoragelosspayment.AnnualSum .= vec(sum(tempesrpayment, dims = 2))
    dfESRStoragelosspayment = hcat(dfESRStoragelosspayment, DataFrame(tempesrpayment, [Symbol("ESR_$i") for i in 1:nESR]))
    CSV.write(string(path, sep, "ESR_StoragelossPayment.csv"), dfESRStoragelosspayment)
