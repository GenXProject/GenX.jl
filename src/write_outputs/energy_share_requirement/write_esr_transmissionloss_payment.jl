...
function write_esr_transmissionlosspayment(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    for i in 1:inputs["nESR"]
	    ...
	    tempesrpayment = ((1/2) .* value.(EP[:eTransLossByZoneYear])) .* inputs["dfESR"][:,Symbol("ESR_$i")] .* dual.(EP[:cESRShare][i])
	    if setup["ParameterScale"] == 1
	        tempesrpayment *= (ModelScalingFactor^2)
	    end
	    dfESRtransmissionlosspayment.AnnualSum .+= tempesrpayment
	    dfESRtransmissionlosspayment = hcat(dfESRtransmissionlosspayment, DataFrame([tempesrpayment], [Symbol("ESR_$i")]))
	end
