...
function write_capacity(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for i in inputs["NEW_CAP"]
		if i in inputs["COMMIT"]
			capdischarge[i] = value(EP[:vCAP][i])*cap_size(i)
		else
	...
	for i in inputs["RET_CAP"]
		if i in inputs["COMMIT"]
			retcapdischarge[i] = first(value.(EP[:vRETCAP][i]))*cap_size(i)
		else
	...
	for y in has_positive_max_capacity_mw(resources)
	...
	for i in inputs["STOR_ASYMMETRIC"]
		...
		existingcapcharge[i] = MultiStage == 1 ? value(EP[:vEXISTINGCAPCHARGE][i]) : existing_charge_capacity_mw(i)
	end
	...
	for i in inputs["STOR_ALL"]
		...
		existingcapenergy[i] = MultiStage == 1 ? value(EP[:vEXISTINGCAPENERGY][i]) :  existing_capacity_mwh(y)
	end
	if !isempty(inputs["VRE_STOR"])
		for i in inputs["VS_STOR"]
			...
			existingcapenergy[i] = existing_capacity_mwh(y) # multistage functionality doesn't exist yet for VRE-storage resources
		end
	end

	dfCap = DataFrame(
		Resource = inputs["RESOURCE_NAMES"], Zone = zone_id.(resources),
		StartCap = MultiStage == 1 ? value.(EP[:vEXISTINGCAP]) : existing_capacity_mw(y),
