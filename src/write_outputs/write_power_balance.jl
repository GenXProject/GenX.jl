function write_power_balance(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    for z = 1:Z
        ...
        if !isempty(intersect(dfGen[dfGen.Zone.==z, :R_ID], FLEX))
            ...
            powerbalance[(z-1)*10+4, :] =
                sum((value.(EP[:vCHARGE_FLEX][FLEX_ZONE, :]).data), dims = 1)
            powerbalance[(z-1)*10+5, :] =
                (-1) * sum(value.(EP[:vP][FLEX_ZONE, :]), dims = 1)
        end
        if SEG > 1
