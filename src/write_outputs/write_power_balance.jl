function write_power_balance(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	for z in 1:Z
		POWER_ZONE = intersect(resources_in_zone_by_rid(resources,z), union(THERM_ALL, VRE, MUST_RUN, HYDRO_RES))
		powerbalance[(z-1)*L+1, :] = sum(value.(EP[:vP][POWER_ZONE, :]), dims = 1)
		if !isempty(intersect(resources_in_zone_by_rid(resources,z), STOR_ALL))
		    STOR_ALL_ZONE = intersect(resources_in_zone_by_rid(resources,z), STOR_ALL)
		...
		if !isempty(intersect(resources_in_zone_by_rid(resources,z), VRE_STOR))
			VS_ALL_ZONE = intersect(resources_in_zone_by_rid(resources,z), inputs["VS_STOR"])
		...
		if !isempty(intersect(resources_in_zone_by_rid(resources,z), FLEX))
		    FLEX_ZONE = intersect(resources_in_zone_by_rid(resources,z), FLEX)
