...
function write_capacity_value(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, dfPower::DataFrame, dfCharge::DataFrame, dfResMar::DataFrame, dfCap::DataFrame)
    ...
    if setup["ParameterScale"] == 1
        existingplant_position = find(x -> x >= 1, (value.(EP[:eTotalCap]).data) * ModelScalingFactor)
    else
        existingplant_position = find(x -> x >= 1, (value.(EP[:eTotalCap]).data))
    end
	...
	for i in 1:inputs["NCapacityReserveMargin"]
		...
		if setup["ParameterScale"] == 1
			riskyhour_position = findall(x -> x >= 1, (transpose(dual.(EP[:cCapacityResMargin][i, :]).data) ./ inputs["omega"] * ModelScalingFactor))
		else
			riskyhour_position = findall(x -> x >= 1, (transpose(dual.(EP[:cCapacityResMargin][i, :]).data) ./ inputs["omega"]))
		end
		...
		temp_capvalue[STOR_ALL_EX, :] = temp_cap_derate[STOR_ALL_EX, :] .* ((value.(EP[:vP][STOR_ALL_EX, :]).data - value.(EP[:vCHARGE][STOR_ALL_EX, :]).data)) .* temp_riskyhour[STOR_ALL_EX, :] ./ totalcap[STOR_ALL_EX, :]
		temp_capvalue[FLEX_EX, :] = temp_cap_derate[FLEX_EX, :] .* ((value.(EP[:vCHARGE_FLEX][FLEX_EX, :]).data - value.(EP[:vP][FLEX_EX, :]).data)) .* temp_riskyhour[FLEX_EX, :] ./ totalcap[FLEX_EX, :]
		temp_dfCapValue.AnnualAverage .= temp_capvalue * inputs["omega"] / sum(inputs["omega"])
		temp_dfCapValue = hcat(temp_dfCapValue, temp_capvalue)
		auxNew_Names_res = [Symbol("Region"); Symbol("Resource"); Symbol("Zone"); Symbol("Cluster"); Symbol("Reserve"); Symbol("AnnualAverage"); [Symbol("t$t") for t in 1:T]]
		rename!(temp_dfCapValue, auxNew_Names_res)
