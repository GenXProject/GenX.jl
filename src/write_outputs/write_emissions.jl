...
function write_emissions(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    if (
        ...
        (
            setup["UCommit"] == 2 && (
                setup["Reserves"] == 0 ||
                (setup["Reserves"] > 0 && inputs["pDynamic_Contingency"] == 0)
            )
        )
    ) # fully linear model
        ...
        for i = 1:Z
            dfEmissions[i, :AnnualSum] =
                sum(inputs["omega"] .* value.(EP[:eEmissionsByZone][i, :])) * scale_factor
        end

        dfEmissions = hcat(
            dfEmissions,
            DataFrame(value.(EP[:eEmissionsByZone]) * scale_factor, :auto),
        )
        ...
        if setup["CO2Cap"] >= 1
            ...
            rename!(dfEmissions, auxNew_Names)
            total = DataFrame(
                ["Total" zeros(1, inputs["NCO2Cap"]) sum(dfEmissions[!, :AnnualSum]) fill(
                    0.0,
                    (1, T),
                )],
                :auto,
        ...
        rename!(dfEmissions, auxNew_Names)
        total =
            DataFrame(["Total" sum(dfEmissions[!, :AnnualSum]) fill(0.0, (1, T))], :auto)
        for t = 1:T
            total[:, t+2] .= sum(dfEmissions[:, Symbol("t$t")][1:Z])
        end
        rename!(total, auxNew_Names)
