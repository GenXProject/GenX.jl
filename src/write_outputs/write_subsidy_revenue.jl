...
function write_subsidy_revenue(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	MAX_CAP = dfGen[(dfGen[!, :Max_Cap_MW].>0), :R_ID]
	dfSubRevenue.SubsidyRevenue[MIN_CAP] .= (value.(EP[:eTotalCap])[MIN_CAP]) .* (dual.(EP[:cMinCap][MIN_CAP])).data
	dfSubRevenue.SubsidyRevenue[MAX_CAP] .= (value.(EP[:eTotalCap])[MAX_CAP]) .* (dual.(EP[:cMaxCap][MAX_CAP])).data
	...
	if (setup["MaxCapReq"] >= 1)
		for maxcap in 1:inputs["NumberOfMaxCapReqs"] # This key only exists if MaxCapReq >= 1, so we can't get it at the top outside of this condition.
			MAX_CAP_GEN = dfGen[(dfGen[!, Symbol("MaxCapTag_$maxcap")].==1), :R_ID]
			dfRegSubRevenue.SubsidyRevenue[MAX_CAP_GEN] .= dfRegSubRevenue.SubsidyRevenue[MAX_CAP_GEN] + (value.(EP[:eTotalCap][MAX_CAP_GEN])) * (dual.(EP[:cZoneMaxCapReq][maxcap]))
		end
	end
