...
discrete new line is its own row (see `expand_discrete_build_lines`), so the solved model has more
...
(only called for `l in DISCRETE_BUILD_LINES`). Passing these as callbacks lets the monolithic and
...
function network_expansion_dataframe(inputs::Dict, setup::Dict;
    ...
    # Map each expanded line to its original corridor (identity when no discrete-build rebuild occurred).
