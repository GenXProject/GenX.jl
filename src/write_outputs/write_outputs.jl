    ...
    if setup["CoolingDemand"] == 1
        ...
        if output_settings_d["WriteUse"] || output_settings_d["WriteNetRevenue"]
            ...
            if setup["withUTES"] >= 1
                ...
                println(elapsed_time_use)
                if !isempty(inputs["STOR_UTES_LONG_DURATION"])
                    elapsed_time_use = @elapsed dfUse = write_opwrap_lds_stor_init_utes(path, inputs, setup, EP)
                    println("Time elapsed for writing LDES SOC (UTES) is")
                    println(elapsed_time_use)
                end
            end
        end
