...
"""
function write_piecewise_fuel(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	dfGen = inputs["dfGen"]
	G = inputs["G"]
	...
	dfFuel = vcat(dfFuel, total)
 	CSV.write(joinpath(path,"piecewise_fuel.csv"), dftranspose(dfFuel, false), writeheader=false)
	return dfFuel
end
