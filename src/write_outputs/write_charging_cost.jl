...
function write_charging_cost(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	if setup["VreStor"] == 1
		...
		dfChargingcostVRESTOR = DataFrame(Region = dfGen_VRE_STOR[!, :region], Resource = inputs["RESOURCES_VRE_STOR"], Zone = dfGen_VRE_STOR[!, :Zone], Cluster = dfGen_VRE_STOR[!, :cluster], AnnualSum = Array{Float64}(undef, VRE_STOR),)
		...
		dfChargingcostVRESTOR.AnnualSum .= chargecost_VRE_STOR * inputs["omega"]
