...
- Build-gated DC-OPF physics for discrete candidate lines: an unbuilt candidate line imposes no flow–angle coupling and no angle-difference limit, so a greenfield corridor constrains phase angles only once at least one circuit is actually built. The zero-capacity residual row of a greenfield discrete corridor is classified as a phantom line and exempted from both, preventing an unbuilt corridor from pinning the bus-angle difference of the two zones it would connect.
